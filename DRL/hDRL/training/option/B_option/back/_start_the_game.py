...
for episodes in range(num_episodes_B1):
    ...
    if r_accumulated_B1 > 0:
        ...
        B_experience_2 = [1, env_B1.mining_powers[env_B1.miner_collude_with]]
        B.myBuffer_B1_2.add(np.reshape(np.array(B_experience_2), [1, input_size_B1_2 + 1]))

    else:
        B_experience_2 = [0, env_B1.mining_powers[env_B1.miner_collude_with]]
        B.myBuffer_B1_2.add(np.reshape(np.array(B_experience_2), [1, input_size_B1_2 + 1]))

    if episodes > 64:
        loss = B.update('B1_1')
        loss_NLL = B.update('B1_2')
        print([episodes, loss, loss_NLL, env_B1.mining_powers[env_B1.miner_collude_with], r_accumulated_B1], file=open(record_path + 'record_B1.txt', 'a+'))
        print('', file=open(record_path + 'record_B1.txt', 'a+'))

    if episodes % 100 == 0 and episodes != 0:
        print("episode: " + str(episodes) + " predict_loss: " + str(loss) + " predict_loss_NLL: " + str(loss_NLL))
        ...
        B.save(path_every, 'B1_2')

    if episodes > 64 and loss < B_min_loss:
        B_min_loss = loss
        B.save(best_model_path, 'B1_1')

    if episodes > 64 and loss_NLL < B_min_loss_NLL:
        B_min_loss_NLL = loss_NLL
        B.save(best_model_path, 'B1_2')
