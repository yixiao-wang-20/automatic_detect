...
class linear_network(nn.Module):
    ...
    def __init__(self, h_size, input_size):
        ...
        self.out1 = nn.Sequential(
            nn.Linear(input_size, int(h_size)),
            nn.BatchNorm1d(int(h_size)),
            nn.ReLU(),
            nn.Linear(int(h_size), int(h_size / 2)),
...
class linear_net():  
    ...
    def predict_the_reward(self, s):
        ...
        Q = self.predict_net(torch.Tensor(np.array(s)).unsqueeze(dim = 0).to(self.device)).squeeze(dim = 0).detach().cpu().numpy()[0]

        return Q

    def update(self, input_vector, r):

        self.predict_net.train()
        input_vector = torch.Tensor(input_vector).to(self.device)
        r = torch.Tensor(r).to(self.device)
        pre = self.predict_net(input_vector).squeeze(dim = 1)
        loss = torch.mean(torch.pow(pre - r, 2) / 100000000)
